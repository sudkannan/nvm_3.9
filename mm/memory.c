...
/*Add NV pages to this list when contents 
 * are freed */
struct list_head nv_free_page_list;
//List containing pages from deleted list
int list_intialized = 0;
int current_vma_id;
unsigned long alloc_nvpages_cnt;
struct pmem_map_struct *pmem_map;
...
//int find_first_and_delete( struct page *page_to_del, struct list_head *head,
//                            struct vm_area_struct *vma );
int find_first_and_delete( struct page *page);
		...
		if (pte_present(ptent)) {
			...
			if (unlikely(page_mapcount(page) < 0)) {
				if(test_bit(PG_nvram, &page->flags))
					printk("NVM page with page count less than 0 \n");
		...
		struct zap_details *details)
{
	...
	//printk(KERN_ALERT "starting unmap_single_vma\n");
	...
	if (start != end) {
		if (unlikely(is_vm_hugetlb_page(vma))) {
			...
			//if(vma->persist_flags == PERSIST_VMA_FLAG) 
			//	printk(KERN_ALERT "before unmap_page_range called \n");

			unmap_page_range(tlb, vma, start, end, details);

			//if(vma->persist_flags == PERSIST_VMA_FLAG) 
			//	printk(KERN_ALERT "after unmap_page_range called \n");
		}
	}
	//if(vma->persist_flags == PERSIST_VMA_FLAG) 
	//printk(KERN_ALERT "exiting unmap_single_vma\n");
}
		...
		unsigned int flags)
{
	...
	if(current && current->heteroflag == PF_HETEROMEM){
       page = NULL;
	   page = hetero_alloc_mm(GFP_HIGHUSER_MOVABLE |__GFP_ZERO, 0, 0);
	   if(page){
		    if(page->nvdirty == PAGE_MIGRATED){
			    clear_user_highpage(page, address);
			...
			printk(KERN_ALERT "memory.c: hetero_alloc_mm failed\n");
		}
	}
#endif
#endif
	...
	//if(page_to_nid(page))
	//printk(KERN_ALERT "page_to_nodeid %d \n",page_to_nid(page));	
		     ...
		     pte_t *pte, pmd_t *pmd, unsigned int flags)
{
	...
	if(vma->persist_flags == PERSIST_VMA_FLAG)
        printk("handle_pte_fault: PERSIST_VMA_FLAG is set \n");
#endif
...
retry:
   ...
   //if(vma->persist_flags == PERSIST_VMA_FLAG)
   //        printk("handle_mm_fault PERSIST_VMA_FLAG is set \n");
...
#ifdef HETEROMEM
			/*If we use hetero support, for nonpersistent memory alloc
			 * We just call the hetero alloc function 
			 * We don't care about the node, handled by the callee*/
			page = hetero_alloc_mm(GFP_HIGHUSER_MOVABLE |__GFP_ZERO, 0, 0);		
#else			
...
#endif
			/*All allocation failed. Revert to regular allocation*/
			if(!page) {
				page = alloc_zeroed_user_highpage_movable(vma, address);
			}
		}
...
write_fault:
	...
	if (unlikely(anon_vma_prepare(vma)))
        ...
        page = NULL;
		//page = hetero_getnxt_page(false);
		page = hetero_alloc_mm(GFP_PERSISTENCE, 0, 0);
		//page = nv_alloc_page_numa(vma);
		if(!page){
			page = alloc_zeroed_user_highpage_movable(vma, address);
			if(!page) {	
...
#ifdef DEBUG_TIMER
   long temp_time = 0;
    struct timespec strt_tv, end_tv;

    if (vma &&  vma->persist_flags == PERSIST_VMA_FLAG ) {
        getnstimeofday (&strt_tv);
		++print_cntr;
    }	
#endif
	pte_unmap(page_table);
	...
	if (!(flags & FAULT_FLAG_WRITE)) {
		...
		if(page) {	
			...
			//printk("do_anonymous_nvmem_page: nv_read succeeded \n");
			entry = mk_pte(page, vma->vm_page_prot);
			// printk("do_anonymous_nvmem_page: after adding entry \n");
		/*}else {

			//printk("do_anonymous_nvmem_page: nv_read failed \n");
			entry = pte_mkspecial(pfn_pte(my_zero_pfn(address),
							vma->vm_page_prot));
		}*/	
			page_table = pte_offset_map_lock(mm, pmd, address, &ptl);
...
write_fault:
	...
	if (unlikely(anon_vma_prepare(vma)))
...
#endif
			if(page) {
				/*I think we don't need to set the PG_nvram bit if 
				* no persistence is required and JIT is enabled*/
#ifdef DEBUG_STATS
				non_persist_pages++;
				printk("total non_persist_pages %u \n",non_persist_pages);
#endif
			}
		}
		...
		/*set the NVRAM dirty bit to true*/
		if(write_fault){
			page->nvdirty =1;
			del_dirtypgcnt++;
			//printk("total dirty page: %u \n",del_dirtypgcnt);
		}
...
#ifdef LOCAL_DEBUG_FLAG
                printk("do_anonymous_nvmem_page: page evicatable \n");
#endif
...
unlock:
    ...
    if (vma &&  vma->persist_flags == PERSIST_VMA_FLAG) {
        ...
        temp_time = simulation_time(strt_tv,end_tv);
        total_time = total_time + temp_time;

        if(print_cntr % 15999 == 0){
            printk("FT %ld \n",total_time);
            print_cntr = 0;
        }
    }
...
int refill_nvpages(void){
	...
	printk("g_nv_init_pgs %u \n", g_nv_init_pgs);

    /*allocated more if less than min threshold */
	if ((g_nv_init_pgs - alloc_nvpages_cnt) < MIN_NVPGS_THRESH){
		if(alloc_fresh_nv_pages( NULL, MIN_NVPGS_THRESH)){ 
			printk("getnvpage:refill failed\n");
			return -1;
        }else {
#ifdef LOCAL_DEBUG_FLAG
 			printk("getnvpage:refill success\n");
			printk("total allocated: %u" 
				   "total used: %ld",g_nv_init_pgs,
					alloc_nvpages_cnt); 
#endif
			g_nv_init_pgs += MIN_NVPGS_THRESH;
		}
	}
	return 0;
}
...
#else
	if(!g_nvlistinit) {	
		 printk("getnvpage: list not intialized \n");
		 goto page_error;
	}

#ifdef LOCAL_DEBUG_FLAG
    printk("getting pages to to free page list \n");	
#endif

    list_for_each_entry_safe(page, tmp, &nv_free_page_list, nvlist)
    if ( page != NULL ) {
	      alloc_nvpages_cnt++;
    	 /*delete the entry from free page list */          
    	 WARN_ON(find_first_and_delete(page));
		/*successful in deleting a page */
		//printk("Successful in deleting pages \n");
		break;
	}else {
		goto page_error;	
	}
...
struct page* get_hetero_io_page(struct vm_area_struct *vma ) {
    ...
    spin_lock(&nvpagelock);
#endif
    //page = hetero_getnxt_io_page_mm(false);
	page = hetero_alloc_mm(GFP_PERSISTENCE, 0, 0);
    if(!page){
...
/**
 * find_persistent_node - find the node with persistent memory
 */
static int find_persistent_node(void)
{
    int num_nodes = 0;
...
#ifndef HETEROMEM	
    return PERSISTENT_MEM_NODE;
#else
	return PERSISTENT_MEM_NODE;

    num_nodes = nr_online_nodes;
    if(!num_nodes)
        return 0;
    ...
    for(nid =0; nid < num_nodes; nid++){
		/*check if pVM node*/
         ...
         if( pgdat && pgdat->pmem_node ==  NODE_PERSIST){
            /*curently no policy in place
            so return the first found node*/
#ifdef LOCAL_DEBUG_FLAG
           printk("find_persistent_node: pVM node %d\n", nid);
#endif
...
//if the passed node id is -1, then allocate a new node
static struct page *nv_alloc_fresh_page_node(int nid, unsigned long size)
{
        struct page *page;
        unsigned int order;
...
#ifdef LOCAL_DEBUG_FLAG
        printk( "nv_alloc_fresh_page_node:  \n");
#endif
        if(nid == -1)
        	nid = find_persistent_node();

        if (order >= MAX_ORDER)
                return NULL;

        //page = alloc_pages_exact_node(nid,  GFP_HIGHUSER_MOVABLE |__GFP_ZERO|GFP_THISNODE, order);
        //Method to allocate from NVRAM
        //FIXME do we need a lock here?????  
        page = alloc_pages_nvram(nid,  GFP_HIGHUSER_MOVABLE |__GFP_ZERO|GFP_THISNODE, order);
        if ( page ) 
#ifdef LOCAL_DEBUG_FLAG
...
#ifdef LOCAL_DEBUG_FLAG 
		if(test_bit(PG_nvram, &page->flags))
       		printk("PG_nvram bit set for page \n");
#endif
		return page;
}
...
int alloc_fresh_nv_pages(nodemask_t *nodes_allowed, int num_pages)
{
    ...
    start_nid = find_persistent_node();
    //start_nid = 1;
    ...
    if ( !g_nvlistinit ) {
        INIT_LIST_HEAD(&nv_free_page_list);
        g_nvlistinit = 1;
    }
...
err_alloc:
    printk(KERN_ALERT 
			"alloc_fresh_nv_pages: failed for %d\n",
			num_pages);
	return -1;
}
...
* caller to check the value of pfn*/
int add_to_used_list( unsigned int pfn){

    struct page *page = NULL;
    unsigned int start_nid = 0;

    //get the persistent memory node id;
    start_nid = find_persistent_node();

    //allocate a page
    page = nv_alloc_fresh_page_node(start_nid, PAGE_SIZE);
    if(!page) {
        printk("add_to_used_list: page creation failed \n");
        goto error;
    }
    printk("page allocation successful \n");
    add_to_free_nvlist(page);
...
* free list. In reality should not allocate them,
* but rather pluck them directly from pmem
* @params: pfn_list - list containing PFN values
* @params: count - count of number of pages
* @return: 0 -> success, -1 -> failure
*/
int load_nv_pages( unsigned int *pfn_list, unsigned int count ){

    unsigned int index =0;
    //FIXME: should be ulong for real PFN
    unsigned int pfn_temp;

    if(!pfn_list){
        printk("load_nv_pages: pfn list is null \n");
        goto error;
    } 
    if( count <= 0) {
        printk("load_nv_pages:the number of pages is %u\n", count);
        goto error; 
    }
    for(index=0; index < count; index++) {

        pfn_temp = pfn_list[index];
      
        if( pfn_temp < 1) {
            printk("invalid pfn %u\n",pfn_temp);
            goto error;
        }
        //curretly this adds to free list instead
        //of used list. FIXME
        if ( add_to_used_list(pfn_temp) < 0 ){
            printk("load_nv_pages: add_to_used_list failed at index %d, "
                    "with pfn_temp %u\n", index, pfn_temp);
            goto error;
        }
    }
    printk("load_nv_pages: loading PFN's successful\n");
    return 0;
...
/*gets all the previously allocated pages of process
 * @params: each process is identified by inode
 * @return: pagelist - list of process pages
 */
int init_proc_nvpages(unsigned long inode) {
    ...
    if( pmem_map ){
        ...
        if( map_cnt > 0){
            ...
            //iterate and find the map values for specific
            //process we are trying to intialize
            for(index=0; index < map_cnt; index++){

                if(pmem_map[index].inode == inode){

                    //looks like we matched the inode
                    printk("found inode && total pages allocated \
                            by process %d \n", pmem_map[index].page_count);

                    if( pmem_map[index].page_count > 0){

                        //The pfn list points to physical frame list used by
                        //process previously
                        pfn_list= pmem_map[index].pfn_list;

                        if( pfn_list ) {
                            /*OK we got the pfn list. Now load those pages
                             too freelist pool
                             FIXME: Ideally it should not be to free list pool
                             but to the allocated list pool.*/
                             page_count = pmem_map[index].page_count;
                             err = load_nv_pages(pfn_list,page_count);
                             if(!err){
                                printk("loading nv pages to free list successful\n");
                             }
                             else {
                                  printk("failure: loading nv pages to free list\n");
                                  goto error;
                             }
                        }else{
                            printk("pmem_map[%d] pfn list null \n",index);
                            goto error;
                        }
                    }else{
                        printk("no page history found \n");
                        printk("pmem_map[%d].page_count is %d\n", index,
                                            pmem_map[index].page_count);
                        goto error;
                    }
                }
        ...
        printk("pmem_map is null \n");
        goto error;
    }
error:
    printk("error getting page count \n");
    return -1;
}
EXPORT_SYMBOL(init_proc_nvpages);
...
struct page *hetero_alloc_mm(gfp_t gfp, int order, int node)
{
	...
	/*overriding callers node choice*/
	if(nodeid != node)
		node = nodeid;

	page = alloc_pages_nvram(node, gfp, 0);
    if(!page) {
		printk(KERN_ALERT "hetero_alloc_mm: alloc_pages_nvram "
				"failed returning page \n");
		goto pagemiss;
	}
	//printk(KERN_ALERT "hetero_alloc_mm: allocating NVRAM page \n");
	if(page->nvdirty != PAGE_MIGRATED){
		goto heteropgmisses;
	}else {
		printk(KERN_ALERT "hetero_alloc_mm: page->nvdirty != PAGE_MIGRATED \n");
	}
...
struct page *hetero_alloc_IO_mm(gfp_t gfp, int order, int node)
{
	...
	//return hetero_alloc_mm(gfp, order, node);
...
heteropgmisses:	 /*got page but not fast page*/
	nr_hetero_page_miss++;
	nr_hetero_io_miss++;
	return page;
...
EXPORT_SYMBOL(hetero_alloc_IO_mm);
...
EXPORT_SYMBOL(hetero_getnxt_io_page_mm);
 ...
 * We return with mmap_sem still held, but pte unmapped and unlocked.
 */
static int do_anonymous_page_node(struct mm_struct *mm, struct vm_area_struct *vma,
		unsigned long address, pte_t *page_table, pmd_t *pmd,
		unsigned int flags)
{
	struct page *page;
	...
	//page = alloc_zeroed_user_highpage_movable(vma, address);
	//printk(KERN_ALERT "allocating from persistent node \n");
	//page = nv_alloc_fresh_page_node(find_persistent_node(),PAGE_SIZE);
	order = get_order(PAGE_SIZE);
    page = alloc_pages_exact_node(find_persistent_node(),  GFP_HIGHUSER_MOVABLE |__GFP_ZERO|GFP_THISNODE, order);
	if (!page)
		goto oom;
