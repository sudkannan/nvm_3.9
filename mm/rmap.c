	...
	struct vm_area_struct *vma, unsigned long address)
{
	...
	if (!PageTransHuge(page)){
		if(test_bit(PG_nvram, &page->flags)){
			__inc_zone_page_state(page, NR_NVM_PERSIST_PAGES);
		}else{
			__inc_zone_page_state(page, NR_ANON_PAGES);
		}
	}
...
#ifdef NVMFLAG
	if(test_bit(PG_nvram, &page->flags)){
	    //printk(KERN_ALERT "page_remove_rmap: map count NVM page %d\n", page_mapcount(page));
		if ((page_mapcount(page) > 0) && !atomic_add_negative(-1, &page->_mapcount))
			goto out;
	}
	else 
#endif
